...
class CookieCaptureGUI:
    def __init__(self, root):
        ...
        self.ip_label = tk.Label(
            root, text="Server IP: " + self.get_ip(), font=("Arial", 12)
        )
        ...
        self.text_area = scrolledtext.ScrolledText(
            root, wrap=tk.WORD, width=80, height=20, font=("Arial", 12)
        )
        ...
        self.watermark = tk.Label(
            root,
            text="Made by Seva41 - Seguridad TI 2024",
            font=("Arial", 8),
            fg="grey",
        )
    ...
    def decrypt_cookie(self, encrypted_cookie):
        try:
            ...
            if encrypted_cookie.startswith("session="):
                encrypted_cookie = encrypted_cookie[len("session=") :]
            # Known MD5 hash to compare
            known_md5 = hashlib.md5(
                "th1s_1s_a_s3cur3_c00ki3".encode("utf-8")
            ).hexdigest()
            print(
                f"Known MD5 hash: {known_md5}"
            )  # Print the known hash for verification
            if encrypted_cookie == known_md5:
                decrypted_cookie = "th1s_1s_a_s3cur3_c00ki3"
            else:
                decrypted_cookie = "Unknown MD5 hash"
